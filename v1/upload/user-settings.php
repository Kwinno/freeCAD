...
if (isset($_POST['updateUserBtn'])) {
	if(empty($_POST['update_password'])) {
		$password = password_hash($_POST['update_password'], PASSWORD_BCRYPT, array("cost" => 12));
		dbquery('UPDATE users SET password="' . $password . '" WHERE user_id="' . escapestring($user_id). '"', false);
